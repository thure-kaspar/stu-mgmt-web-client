...
export class ParticipantAssessmentsEffects {
	loadAssessments$ = createEffect(() =>
		this.actions$.pipe(
			...
			switchMap(([action, user]) =>
				this.userApi.getAssessmentsOfUserForCourse(user.id, action.courseId).pipe(
	...
	constructor(private actions$: Actions, private store: Store, private userApi: UserApi) {}
}
