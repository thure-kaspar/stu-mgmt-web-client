...
export class UserManagementComponent extends UnsubscribeOnDestroy implements OnInit {
	...
	editUser(user: UserDto): void {
		this.subs.sink = this.dialog.open<UpdateUserDialog, UserDto, UserDto>(UpdateUserDialog, { data: user }).afterClosed().subscribe(
			updated => {
				if (updated) {
	...
	removeUser(user: UserDto): void {
		this.subs.sink = this.dialogService.openConfirmDialog({
			title: "Action.Delete",
