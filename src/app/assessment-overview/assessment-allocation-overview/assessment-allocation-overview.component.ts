...
export class AssessmentAllocationOverviewComponent implements OnInit {
	...
	openCopyFromPreviousDialog(): void {
		...
		this.dialog.open<SearchAssignmentDialog, string, AssignmentDto[]>(SearchAssignmentDialog, { data })
			.afterClosed().subscribe(
				selection => {
					const selected = selection?.length > 0 ? selection[0] : undefined;
					if (selected) {
						this.allocationService.addAllocationsFromExistingAssignment(this.courseId, this.assignmentId, selected.id)
							.subscribe(
								result => {
