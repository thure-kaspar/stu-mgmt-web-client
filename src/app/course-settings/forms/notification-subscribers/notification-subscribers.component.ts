...
import { NotificationService, SubscriberDto } from "../../../../../api";
import {
	ConfirmDialog,
	ConfirmDialogData
...
export class NotificationSubscribersComponent implements OnInit {
	...
	ngOnInit(): void {
		this.subscribers$ = combineLatest([this.route.params, this.reload$]).pipe(
			switchMap(([{ courseId }]) => {
				...
				return this.notificationService.getSubscribers(courseId);
			})
		);
	}
	...
	remove(subscriber: SubscriberDto): void {
		...
		this.dialog
			...
			.subscribe(confirmed => {
				if (confirmed) {
					this.notificationService.unsubscribe(this.courseId, subscriber.name).subscribe({
						next: result => {
							this.toast.success();
